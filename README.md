...
или psql с локального компа) — SSH-туннель до хоста сервера. Удалённый порт —
это host-порт из `POSTGRES_PORT_BIND` (например, если задано
`127.0.0.1:5433:5432` — туннель на 5433):

```bash
ssh -L 5433:127.0.0.1:5433 <пользователь>@<хост-сервера>
psql -h 127.0.0.1 -p 5433 -U postgres
```

Если порт хоста свободен (нет других стеков на 5432), удобно
`POSTGRES_PORT_BIND=127.0.0.1:5432:5432` и туннель на 5432.
